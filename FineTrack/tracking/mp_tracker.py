...
from FineTrack.algorithms.shared.utils import add_to_means
...
class MPTracker(object):
    ...
    def track(
        ...
        for rank in range(self.n_processes):
            if rank == 0:
                p = mp.Process(target=self._track_mp, args=(env, tracts_format))
            else:
                cloned_env = env.clone()
                p = mp.Process(target=self._track_mp, args=(cloned_env, tracts_format))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
