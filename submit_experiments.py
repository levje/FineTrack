...
if [ $islocal -eq 0 ]; then
    ...
    if [ -f ~/.comet_api_key ]; then
        echo "Comet API key found, setting up environment variables..."
        export COMET_API_KEY=$(cat ~/.comet_api_key)
    else
        echo "Comet API key not found. Please create a file at ~/.comet_api_key with your Comet API key."
    fi
    
    export COMET_GIT_DIRECTORY="/home/levj1404/FineTrack"
