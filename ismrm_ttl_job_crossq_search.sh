...
# Check if we should perform a hyperparameters search or not
# and change the script used accordingly.
if [ $SEARCH -eq 1 ]; then
    echo "Performing hyperparameters search. Some parameters will be overwritten."
    SCRIPT="FineTrack/searchers/cross_q_searcher.py"
else
    SCRIPT="FineTrack/trainers/cross_q_train.py"
fi
